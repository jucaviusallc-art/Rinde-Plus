    ...
    <div className="max-w-4xl mx-auto space-y-6 py-2">
      <div>
        ...
        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
          Registro histórico de tus compras finalizadas con tasa BCV aplicada
        </p>
      </div>
      ...
      {history.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-10 text-center space-y-3">
          ...
          <p className="text-slate-500 text-sm max-w-sm mx-auto">
            Cuando completes una compra desde tu carrito, aparecerá detallada aquí.
          </p>
        </div>
      ) : (
                  ...
                  <div className="flex items-start sm:items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-lg font-black text-slate-900 dark:text-white mt-0.5">
                        Bs {record.total_bs.toLocaleString("es-VE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        <span className="text-xs text-slate-500 dark:text-slate-400 font-medium ml-2">
                          (${record.total_usd.toFixed(2)} USD)
                        </span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs bg-white dark:bg-slate-800 p-3.5 rounded-2xl border border-slate-200/60 dark:border-slate-700/60">
                      <div>
                        ...
                        <strong className="text-slate-900 dark:text-white">
                          Bs {record.budget_bs.toLocaleString("es-VE", { minimumFractionDigits: 2 })}
                        </strong>
                      </div>
                      <div>
                        <span className="text-slate-400 block">Sobrante al finalizar:</span>
                        <strong className="text-[#2E7D32] dark:text-emerald-400">
                          Bs {record.remaining_bs.toLocaleString("es-VE", { minimumFractionDigits: 2 })}
                        </strong>
                      </div>
                    </div>
                          ...
                          >
                            <div>
                              ...
                              <span className="text-xs text-slate-500">
                                {item.quantity} ud × ${item.price_usd.toFixed(2)} USD (Bs {item.price_bs.toFixed(2)})
                              </span>
                            </div>
                            <div className="text-right font-bold text-slate-900 dark:text-white">
                              Bs {item.subtotal_bs.toLocaleString("es-VE", { minimumFractionDigits: 2 })}
