...
import { Budget, ExchangeRateInfo, RateType } from "../types";
...
export const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  const [tasaCustomStr, setTasaCustomStr] = useState<string>(
    budget?.tasa_custom ? budget.tasa_custom.toString() : "75.00"
  );
  ...
  const bcvRate = rateInfo?.rate || 72.5;
  const parsedMontoBs = parseFloat(montoBsStr) || 0;
  const parsedCustomRate = parseFloat(tasaCustomStr) || bcvRate;
  const activeRate = tipoTasa === "bcv" ? bcvRate : parsedCustomRate;

  const usdEquivalent =
    activeRate > 0 && parsedMontoBs > 0
      ? (parsedMontoBs / activeRate).toFixed(2)
      : "0.00";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (parsedMontoBs <= 0) {
      setErrorMsg("Por favor ingrese un presupuesto válido en Bs mayor a cero.");
      return;
    }

    if (tipoTasa === "custom" && parsedCustomRate <= 0) {
      setErrorMsg("Por favor ingrese una tasa personalizada válida.");
      return;
    }

    try {
      setIsSubmitting(true);
      await onSaveBudget(parsedMontoBs, tipoTasa, parsedCustomRate);
    } catch (err: any) {
    ...
    <div className="max-w-3xl mx-auto space-y-6 py-4">
      ...
      <div className="bg-linear-to-br from-[#2E7D32]/10 via-emerald-50 to-white dark:from-emerald-950/40 dark:via-slate-900 dark:to-slate-900 border border-emerald-200/80 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tight">
            Rinde<span className="text-[#2E7D32] font-black">+</span>
          </h1>
        </div>
        <p className="text-slate-600 dark:text-slate-300 text-sm sm:text-base max-w-xl">
          Controla tu presupuesto de compras en tiempo real en Venezuela.
          Convierte Bs a USD instantáneamente usando la tasa oficial del Banco Central de Venezuela.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Presupuesto Inicial en Bs Input */}
          <div>
          ...
          <div className="space-y-3">
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">
              Selecciona la Tasa de Cambio (Bs/USD)
            </label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                <div className="flex items-center justify-between w-full mb-1">
                  <span className="font-bold text-sm flex items-center gap-1.5">
                    <TrendingUp className="w-4 h-4 text-[#2E7D32]" /> Tasa
                    Oficial BCV
                  </span>
                  <input
                    type="radio"
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                  Obtenida del Banco Central de Venezuela
                </p>
              </button>
              ...
              >
                <div className="flex items-center justify-between w-full mb-1">
                  <span className="font-bold text-sm">Tasa Personalizada</span>
                  <input
                    type="radio"
                    name="rate_type"
                ...
                <div className="mt-2 text-sm font-semibold text-slate-600 dark:text-slate-300">
                  Ingresa tu propia tasa de cambio manual
                </div>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                  Útil para acordar tasas de tienda o paralelo
                </p>
              </button>
            </div>
          </div>

          {/* Manual Custom Rate Field (shown if Custom selected) */}
          {tipoTasa === "custom" && (
            <div className="p-4 bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/60 rounded-2xl space-y-2 animate-fadeIn">
              <label className="block text-sm font-semibold text-amber-900 dark:text-amber-200">
                Ingresa el valor de la tasa manual (Bs por 1 USD)
              </label>
              <div className="relative">
